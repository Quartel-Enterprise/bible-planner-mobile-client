...
1. `LocalNavigationBarInsets` (ui/utils) is provided in `AppRoot.kt` root — capturing the live `WindowInsets.navigationBars` object from the Activity window — and consumed via `windowInsetsPadding(LocalNavigationBarInsets.current)` in `CloseableContent`.
