...
metadata: 
  ...
  modified: 2026-09-13T19:10:35.151Z
---
...
Workaround in PR #437 (2026-09-13): `WindowInsets.asStable()` in ui/utils snapshots values into value-equal `WindowInsets(Int…)`, applied to the reader screens/bars. Reported to JetBrains as https://youtrack.jetbrains.com/issue/CMP-10789.

**Why:** screens moved between Nav3 scenes must not feed unstable insets to inset modifiers on iOS.
**How to apply:** use `asStable()` for insets passed to windowInsetsPadding/Scaffold/TopAppBar/BottomAppBar in any screen that participates in a custom Scene (DayScreen/DayStudyScreen in DayStudyPanelScene still don't). When bumping CMP, check CMP-10789 / the androidx fix; if included, drop the helper. Related: [[project-navigation3-migration]].
