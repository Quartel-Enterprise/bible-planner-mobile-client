...
- The same `init` block also reports a RevenueCat custom paywall impression (`TrackCustomPaywallImpression`), which is not analytics — it feeds RevenueCat's exposure data for Experiments. See [setup_revenuecat.md](../../setup_revenuecat.md#6-custom-paywall-impressions).
