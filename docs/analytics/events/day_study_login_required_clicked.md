...
Captures the user interacting with the Day Study card while signed out, triggering the login-required warning. This click also triggers navigation to the login-warning dialog (which `screen_view` logs separately), so this event isolates click-through intent from the resulting screen-view.
