...
Captures the user tapping a chapter to open it on the Read screen. Because this click also triggers navigation (which `screen_view` logs separately), this event isolates click-through intent — which chapters get tapped, from which surface — from the resulting screen-view. Fired from two different surfaces.
...
- Related: [screen_view](screen_view.md) (`read` with `book_id`, `chapter_number`) fires separately once the destination becomes visible.
