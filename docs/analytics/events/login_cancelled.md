...
- Dismissing the login **bottom sheet** itself (`DismissClick` / `NotNowClick`) is not tracked as a cancel — no native flow was started; the sheet impression is covered by `screen_view`.
