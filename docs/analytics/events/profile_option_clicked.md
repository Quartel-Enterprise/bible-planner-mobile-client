...
Captures which entry the user taps in the Profile tab, the app's settings and utilities hub. It reveals which secondary features (donation, web app, release notes, support, etc.) actually get discovered and used, and feeds the entry step of several funnels (monetization via `donate`, support via `contact_support`).

## When it fires

User taps any list item on the Profile screen.

## Trigger source

`feature/profile/src/commonMain/kotlin/com/quare/bibleplanner/feature/profile/presentation/viewmodel/ProfileViewModel.kt` — `ProfileUiEvent.OnItemClick(type: ProfileOptionItemType)`
