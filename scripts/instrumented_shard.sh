...
# The instrumented-tests workflow runs every Android shard on its own emulator. Modules are
# discovered by their src/androidDeviceTest directory, the same one that makes build-logic run
# their commonTest on a device, and dealt out round-robin, so a new module joins a shard without
...
if [ "$#" -ne 3 ] || { [ "$1" != "assemble" ] && [ "$1" != "connected" ] && [ "$1" != "ios" ]; }; then
    echo "Usage: $0 <assemble|connected|ios> <shard-index> <shard-count>" >&2
    exit 1
fi
